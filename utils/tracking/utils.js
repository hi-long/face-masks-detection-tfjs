export const isDetectionTooLarge = (detections, largestAllowed) => {
  return (detections.w >= largestAllowed)
}

export const isInsideArea = (area, point) => {
...
export const isInsideSomeAreas = (areas, point) => {
  return areas.some((area) => isInsideArea(area, point));
}

export const ignoreObjectsNotToDetect = (detections, objectsToDetect) => {
  return detections.filter((detection) => objectsToDetect.indexOf(detection.name) > -1)
}

export const getRectangleEdges = (item) => {
  return {
    x0: item.x - item.w / 2,
    y0: item.y - item.h / 2,
    x1: item.x + item.w / 2,
...
export const iouAreas = (item1, item2) => {
...
export const computeVelocityVector = (item1, item2, nbFrame) => {
  return {
...
export const computeBearingIn360 = function(dx,dy) {
