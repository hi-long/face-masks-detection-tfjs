import { computeBearingIn360 } from './utils';
import { computeVelocityVector } from './utils';

export const ITEM_HISTORY_MAX_LENGTH = 15;

// Use a simple incremental unique id for the display
var idDisplay = 0;

export const ItemTracked = (properties, frameNb, unMatchedFramesTolerance, fastDelete) => {
  ...
  itemTracked.updateVelocityVector = function() {
    ...
    if(this.itemHistory.length <= ITEM_HISTORY_MAX_LENGTH) {
      ...
      const start = this.itemHistory[this.itemHistory.length - ITEM_HISTORY_MAX_LENGTH];
      const end = this.itemHistory[this.itemHistory.length - 1];
