import { ItemTracked as _ItemTracked, resetItem } from "./ItemTracked";
var ItemTracked = _ItemTracked;
import { kdTree } from '../../lib/kdTree-min';
import { iouAreas } from './utils';
...
export const computeDistance = iouDistance;

export function updateTrackedItemsWithNewFrame(detectionsOfThisFrame, frameNb) {
  ...
  console.log(detectionsOfThisFrame, frameNb);
...
export function enableKeepInMemory() {
  keepAllHistoryInMemory = true;
}

export function disableKeepInMemory() {
  keepAllHistoryInMemory = false;
}

export function getJSONOfTrackedItems (roundInt = true)
{
...
export function getJSONDebugOfTrackedItems(roundInt = true) {
  return Array.from(mapOfItemsTracked.values()).map(function(itemTracked) {
    return itemTracked.toJSONDebug(roundInt);
  });
}

export function getTrackedItemsInMOTFormat(frameNb) {
  return Array.from(mapOfItemsTracked.values()).map(function(itemTracked) {
...
// Work only if keepInMemory is enabled
export function getJSONOfAllTrackedItems() {
