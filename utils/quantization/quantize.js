...
export default function quantizeWeights(tensor, qdtype = 0) {
  ...
  if (min === max) {
    return {
      ...
      qdata:
        qdtype === 0
          ? new Uint8Array(tensor.size)
          : new Uint16Array(tensor.size),
    };
  }
  ...
  const qdata = tf
    .round(
      tf
        .clipByValue(tensor, q.min, q.max)
        .sub(tf.scalar(q.min))
        .div(tf.scalar(q.scale))
    )
    .dataSync();
